...
let fallSum: number = 0;
for (let i = 0; i < BRICKS.length; i++) {
	BRICKS.forEach ( brick => { brick.falling = false })
	BRICKS[i].falling = true;
	let toVisit: Brick[] = BRICKS[i].aboveNeighbour;
	while (toVisit.length != 0) {
		let first = toVisit.shift();
		if (first === undefined) {continue};
		let toFall: boolean = true;
		first.belowNeighbour.forEach ( below => {
		...
		if (!toFall) {
			...
			first.falling = true;
			fallSum += 1;
			first.aboveNeighbour.forEach ( (above) => {
				if (! toVisit.includes(above) ) {
					toVisit.push(above);
				}
			});
		};
